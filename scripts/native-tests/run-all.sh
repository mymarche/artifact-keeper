...
case "$PROFILE" in
    ...
    proxy)
        TESTS=("proxy-virtual")
        ;;
    pypi|npm|cargo|maven|go|rpm|deb|helm|conda|docker|protobuf|incus|hex|pub|proxy-virtual|pub-proxy|pub-virtual|health-probes|tag-replication|curation)
        TESTS=("$PROFILE")
        ;;
    *)
        echo "ERROR: Unknown profile: $PROFILE"
        echo "Available profiles: smoke, all, pypi, npm, cargo, maven, go, rpm, deb, helm, conda, docker, protobuf, incus, hex, pub, proxy, pub-proxy, pub-virtual, tag-replication, curation"
        exit 1
        ;;
esac

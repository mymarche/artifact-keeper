...
async fn handle_get(
    ...
    match request {
        ...
        GoProxyRequest::Mod { module, version } => {
            get_mod_file(&state, &repo, &module, &version, client_ip.as_str(), user_agent.as_str()).await
        }
        GoProxyRequest::Zip { module, version } => {
            download_zip(&state, &repo, &module, &version, client_ip.as_str(), user_agent.as_str()).await
        }
